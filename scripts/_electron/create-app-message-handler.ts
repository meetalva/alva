	...
	return async function appMessageHandler(message: Message.Message): Promise<void> {
		switch (message.type) {
			...
			case Message.MessageType.AppLoaded: {
				// TODO: Make this window/project specific?
			...
			case Message.MessageType.WindowOpen: {
				if (!ctx.port) {
					return;
				}

				const { projectId, windowId } = message.payload;

				const alvaWindow = new AlvaWindow({
					id: windowId,
					projectId,
					port: ctx.port
				});

				ctx.windows.set(alvaWindow.id, alvaWindow);

				break;
			}
			case Message.MessageType.WindowClose: {
				// TODO: Scope to passed window id
